/**
 * Script to link a specific Stripe subscription to a specific user
 * Using exact values provided by the user
 */
...
const CUSTOMER_EMAIL = 'handscrapedflooring@gmail.com'; // Corrected email
const USER_ID = '1f75b605-e93c-480c-8e34-0687e88f0d12';   // Exact UUID from Supabase

async function linkSpecificSubscription() {
  console.log(`\n----- LINKING SPECIFIC SUBSCRIPTION -----`);
  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      console.log(`\nTrying broader search in Stripe...`);
      const allCustomers = await stripe.customers.list({ limit: 100 });
      
      console.log(`Found ${allCustomers.data.length} total customers in Stripe:`);
      allCustomers.data.forEach((cust, i) => {
        console.log(`${i + 1}. ID: ${cust.id}, Email: ${cust.email || 'None'}, Name: ${cust.name || 'None'}`);
      });
